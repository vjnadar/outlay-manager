...
export const setEntries = (values, selectedDate, _id) => {
  let entries = {
    _id: _id ? _id : "",
...
export const findIfCustom = (value, type) => {
  let result;
  if (type === "income") {
...
export const outlayFormValidationSchema = () => {
  const validationSchema = Yup.object().shape({
    ...
    iCustom: Yup.string().when("itype", {
      is: "Custom",
      then: Yup.string()
        .max(15, "The label should not be more than 15 letters long.")
        .required("This field is required and must not be empty."),
      otherwise: Yup.string()
        .ensure()
        .min(0)
        .notRequired()
    ...
    amount: Yup.number()
      ...
      .required("This field is required and must be filled.")
  });
